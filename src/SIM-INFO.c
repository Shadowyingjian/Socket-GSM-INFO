...
/*
+MONI:<netname>,BSIC:<bsic>,RxQual:<qual>,LAC:<lac>,Id:<id>,ARFCN:<arfcn>,PWR:<dBm>,C1:<C1>-C2:<C2>
+MONI: Cc: 460,Nc: 00,BSIC: 0,RXQual: 255,LAC: 9540,Id: 41228,ARFCN: 42,PWR: -85dBm,C1: 18-C2: 128
*/
typedef struct Moni_INFO{
	char CC[20];
	char NC[20];
	char BSIC[20];
	char RXQUAL[20];
...
*
******************************************************/
int Get_MONI_Data(char *o_data,char *w_data,char *w2_data,char r_data[100])
{
     int result = 0;
	 int len = 0;
	 char *g1 = strstr(o_data,w_data);
...
+MONI: Cc: 460,Nc: 00,BSIC: 0,RXQual: 255,LAC: 9540,Id: 41228,ARFCN: 42,PWR: -85dBm,C1: 18-C2: 128
*/
int Get_M_Data(char *o_data,struct Moni_INFO *m_info)
{
	int result = 0;
	Get_MONI_Data(o_data,"Cc:","Nc:",m_info->CC);
	result = 1;
    Get_MONI_Data(o_data,"Nc:","BSIC:",m_info->NC);
	result = 2;
    Get_MONI_Data(o_data,"BSIC:","RXQual:",m_info->BSIC);
	result = 3;
	Get_MONI_Data(o_data,"RXQual:","LAC:",m_info->RXQUAL);
	result = 4;
	Get_MONI_Data(o_data,"LAC:","Id:",m_info->LAC);
	result = 5;
	Get_MONI_Data(o_data,"Id:","ARFCN:",m_info->ID);
	result = 6;
	Get_MONI_Data(o_data,"ARFCN:","PWR:",m_info->ARFCN);
	result = 7;
	Get_MONI_Data(o_data,"PWR:","C1:",m_info->PWR);
	result = 8;
	Get_MONI_Data(o_data,"C1:","C2:",m_info->C1);
	result = 9;
	Get_MONI_Data(o_data,"C2:","OK",m_info->C2);
	result = 10;
...
/*************************************************
*  Name:File_Read
*  Function: Read file and get data
*  Input: filename---->the name of file need to read data
*  Output: the len of read
*
*************************************************/
int File_Read(char *filename,char data[4096])
{
    int len;
    int ret;
    int i;
   ...
   if((fd=open(filename,O_RDWR,S_IRWXU)) ==-1)
    ...
    {
        printf("Read file-open file success \n");
    }
    ...
    if(len <=4096)
    {
       ...
       printf("copy data from read_buf\n");
     }
    //to print data 
    for(i=0;i<len;i++)
    {   
...
/*************************************************
* Name: get_file_size
* Function: get file size before write data when file size is bigger than the number then create the file
*
* Input :file_neme the file you need to get it's size.
...
/*********************************************************
*  Name: M_Write
*  Function: to write data to file add time behind data and this is over all data in the file.
*  Input:  file_name----->the file in the directory 
*          data --------->the data want to write in the data.
*  Output:	the length of data have wrote in the file.
*
**********************************************************/
    		...
    		{   
                ...
                switch(step)
                {
                    ...
                    case 0: 
							...
							step = Send_AT_Command(fd,"AT\r\n","OK","ERROR",0,err_count,99,0,test_data,0,0,0); 
							break;
							}
                    ...
                    case 7: 
							{
								step = Send_AT_Command(fd,"AT+MONI?\r\n","OK","ERROR",step,err_count,1023,7,gsm_name,0,1023,1);
                    ...
                    case 9: {
							step = Send_AT_Command(fd,"AT+CMGSI=4\r\n","OK","ERROR",step,err_count,1023,9,gsm_ccid,0,1023,1); 
							Find_Space(gsm_ccid,'\r','\n','-');  
					...
					case 15:  step = Send_TCPDATA_Command(fd,gsm_data,"OK",1023,step); break;
